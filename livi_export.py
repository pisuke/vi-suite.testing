...
def radgexport(export_op, node, **kwargs):
    ...
    for frame in range(scene.fs, scene.gfe + 1): 
        ...
        if frame in range(scene.fs, max(node['frames']['Geometry'], node['frames']['Material']) + 1):
            ...
            for o in set(geooblist + caloblist + lightlist):
                bm = bmesh.new()
                bm.from_mesh(o.data)
                ...
                elif o.name in scene['livil']:
                    if frame in range(node['frames']['Lights'] + 1):
                        iesname = os.path.splitext(os.path.basename(o.ies_name))[0]
                        subprocess.call("ies2rad -t default -m {0} -c {1[0]:.3f} {1[1]:.3f} {1[2]:.3f} -l / -p {2} -d{3} -o {4}-{5} {6}".format(o.ies_strength, o.ies_colour, scene['viparams']['newdir'], o.ies_unit, iesname, frame, o.ies_name), shell=True)
                        if o.type == 'LAMP':
                            if o.parent:
                                o = o.parent
                            lradfile += "!xform -rx {0} -ry {1} -rz {2} -t {3[0]} {3[1]} {3[2]} {4}.rad\n\n".format((180/pi)*o.rotation_euler[0] - 180, (180/pi)*o.rotation_euler[1], (180/pi)*o.rotation_euler[2], o.location, os.path.join(scene['viparams']['newdir'], iesname+"-{}".format(frame)))
                        elif o.type == 'MESH':
                            rotation = o.rotation_euler
                            for face in bm.faces:
                                (fx, fy, fz) = face.calc_center_median
